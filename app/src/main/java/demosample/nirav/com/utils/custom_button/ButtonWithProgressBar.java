...
public class ButtonWithProgressBar extends android.support.v7.widget.AppCompatButton implements AnimatedButton, ICustomizeUI {
    private enum State {
    ...
    private AnimatedDrawable mAnimatedDrawable;
    private RevealAnimatedDrawable mRevealDrawable;
     ...
     * @param attrs
     */
    public ButtonWithProgressBar(Context context, AttributeSet attrs) {
        super(context, attrs);
     ...
     * @param defStyleAttr
     */
    public ButtonWithProgressBar(Context context, AttributeSet attrs, int defStyleAttr) {
     ...
     * @param attrs attributes for this
     * @param defStyleAttr
     * @param defStyleRes
     */
    @TargetApi(23)
    public ButtonWithProgressBar(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(context, attrs, defStyleAttr);
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes){
        ...
        if(attrs == null) {
            drawables = loadGradientDrawable(ViewUtils.getDrawable(getContext(), R.drawable.shape_default));
        } else{
            int[] attrsArray = new int[] {
                    android.R.attr.background, // 0
            };

            TypedArray typedArray =  context.obtainStyledAttributes(attrs, R.styleable.ButtonWithProgressBar, defStyleAttr, defStyleRes);
            ...
            mParams.mInitialCornerRadius = typedArray.getDimension(
                    R.styleable.ButtonWithProgressBar_initialCornerAngle, 0);
            mParams.mFinalCornerRadius = typedArray.getDimension(
                    R.styleable.ButtonWithProgressBar_finalCornerAngle, 100);
            mParams.mSpinningBarWidth = typedArray.getDimension(
                    R.styleable.ButtonWithProgressBar_progress_bar_width, 10);
            mParams.mSpinningBarColor = typedArray.getColor(R.styleable.ButtonWithProgressBar_progress_bar_color,
                    ViewUtils.getColorWrapper(context, android.R.color.black));
            mParams.mPaddingProgress = typedArray.getDimension(R.styleable.ButtonWithProgressBar_progress_bar_padding, 0);
	...
	private void drawProgress(Canvas canvas) {
        if (mAnimatedDrawable == null || !mAnimatedDrawable.isRunning()) {
            mAnimatedDrawable = new AnimatedDrawable(this,
    ...
    public void setProgress(int progress) {
        progress = Math.max(AnimatedDrawable.MIN_PROGRESS,
                Math.min(AnimatedDrawable.MAX_PROGRESS, progress));
    ...
    public void resetProgress() {
        this.progress = AnimatedDrawable.MIN_PROGRESS - 1;
    }
    ...
    public void stopAnimation(OnAnimationEndListener onAnimationEndListener){
    ...
    public void doneLoadingAnimation(int fillColor, Bitmap bitmap) {
        ...
        mRevealDrawable = new RevealAnimatedDrawable(this, fillColor,bitmap);
    ...
    public void doneLoadingAnimation(int fillColor,int drawableRes)
    {
        Bitmap bitmap = getBitmapFromVectorDrawable(drawableRes);
    ...
    {
        new Handler().postDelayed(() -> revertAnimationWithDelay(onAnimationEndListener),2000);
    }


    private void revertAnimationWithDelay(final OnAnimationEndListener onAnimationEndListener){
        ...
        mAnimatorSet.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                if (doneWhileMorphing) {
                    ...
                    Runnable runnable = () -> doneLoadingAnimation(mFillColorDone, mBitmapDone);
