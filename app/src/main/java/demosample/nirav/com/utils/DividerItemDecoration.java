...
public class DividerItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public DividerItemDecoration(Drawable divider,int avoidLayoutResId) {
        mDivider = divider;
        this.avoidLayoutResId = avoidLayoutResId;
    }
    ...
    private void drawVerticalDividers(Canvas canvas, RecyclerView parent) {
        ...
        for (int i = 0; i < childCount - 1; i++) {
            View child = parent.getChildAt(i);
            View avoidView = child.findViewById(avoidLayoutResId);

            if(avoidView != null){
                width = avoidView.getMeasuredWidth();
                width+= avoidView.getPaddingLeft();
            }
            RecyclerView.LayoutParams params = (RecyclerView.LayoutParams) child.getLayoutParams();

            int parentTop = child.getBottom() + params.bottomMargin;
            int parentBottom = parentTop + mDivider.getIntrinsicHeight();
            mDivider.setBounds(new Rect());
            mDivider.setBounds(width+parentLeft, parentTop, parentRight, parentBottom);
            mDivider.draw(canvas);
        }
    }
}
